...
export default function PhotosUploader({addedPhotos, onChange}){
    ...
    async function addPhotoByLink(ev) {
        ev.preventDefault();
        const {data: filename} = await axios.post('https://backend-booking-two.vercel.app/upload-by-link' , {link: photoLink});
    ...
    function uploadPhoto(ev) {
        ...
        axios.post('https://backend-booking-two.vercel.app/upload', data, {
        ...
        <div className="mt-2 grid gap-2 grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
            {addedPhotos.length > 0 && addedPhotos.map((link, index) => (
                <div className="h-32 flex relative" key={index}>
                    <img className="rounded-2xl w-full object-cover" src={`https://backend-booking-two.vercel.app/${link}`} alt="" />
